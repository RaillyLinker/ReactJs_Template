      ...
      <GcoDialogFrame business={mainBusiness.gcoDialogFrameBusiness}>
        <GcoOuterFrame business={mainBusiness.gcoOuterFrameBusiness} >
          <div id={styles.MainContent}>
            <h3>압축 파일을 선택하면 압축이 풀립니다.</h3>
            <input type="file" accept=".zip" onChange={mainBusiness.handleFileChange} />
            {mainBusiness.error && <p style={{ color: 'red' }}>{mainBusiness.error}</p>}
            <ul>
              {mainBusiness.files.map((file, index) => (
                <li key={index} style={{ marginTop: '10px', cursor: 'pointer', textDecoration: 'underline', color: 'blue' }}
                  onClick={() => mainBusiness.handleFileDownload(file)}>
                  {file.name}
                </li>
              ))}
            </ul>
