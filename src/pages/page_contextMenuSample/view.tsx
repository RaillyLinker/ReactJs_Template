...
const View: React.FC = () => {
  ...
  // (컴포넌트 화면 구성 코드)
  return (
    <div id={styles.MainView} onContextMenu={(e) => mainBusiness.handleRightClick(e, 'default')} onClick={mainBusiness.clickOuter}>
      <GcoDialogFrame business={mainBusiness.gcoDialogFrameBusiness}>
        <GcoOuterFrame business={mainBusiness.gcoOuterFrameBusiness} >
          <div id={styles.MainContent}>
            <div id={styles.RightClickHereArea} onContextMenu={(e) => mainBusiness.handleRightClick(e, 'RightClickHereArea')}>우클릭 하세요</div>
          </div>
          <ToastContainer
            newestOnTop={mainBusiness.toastNewestOnTop}
            rtl={mainBusiness.toastRightToLeftLayout}
      ...
      */
      ...
      {mainBusiness.customContextMenuVisible && (
        <div
          style={{
            position: 'absolute',
            top: `${mainBusiness.customContextMenuPosition.y}px`,
            left: `${mainBusiness.customContextMenuPosition.x}px`,
        ...
        >
          <div
            ...
            onClick={() => mainBusiness.handleMenuClick('back')}
          >
            뒤로가기
          </div>
          <div
            style={{ padding: '8px', cursor: 'pointer' }}
            onClick={() => mainBusiness.handleMenuClick('alert')}
          >
            알림 호출
          </div>
          <div
            style={{ padding: '8px', cursor: 'pointer' }}
            onClick={() => mainBusiness.handleMenuClick('close')}
          >
            닫기
          </div>
        </div>
